...
1 Berkeley Artificial Intelligence Research, University of California, Berkeley, USA. Correspondence to: Tuomas Haarnoja &lt; haarnoja@berkeley.edu &gt; .
...
We explore how to design an efficient and stable modelfree deep RL algorithm for continuous state and action spaces. To that end, we draw on the maximum entropy framework, which augments the standard maximum reward reinforcement learning objective with an entropy maximization term (Ziebart et al., 2008; Toussaint, 2009; Rawlik et al.,
...
Standard RL maximizes the expected sum of rewards ∑ t E ( s t , a t ) ∼ ρ π [ r ( s t , a t )] . We will consider a more general maximum entropy objective (see e.g. Ziebart (2010)), which favors stochastic policies by augmenting the objective with the expected entropy of the policy over ρ π ( s t ) :
...
Lemma 2 (Soft Policy Improvement) . Let π old ∈ Π and let π new be the optimizer of the minimization problem defined in Equation 4. Then Q π new ( s t , a t ) ≥ Q π old ( s t , a t ) for all ( s t , a t ) ∈ S × A with |A| &lt; ∞ .
...
where ϵ t is an input noise vector, sampled from some fixed distribution, such as a spherical Gaussian. We can now rewrite the objective in Equation 10 as

<!-- formula-not-decoded -->

where π φ is defined implicitly in terms of f φ , and we have noted that the partition function is independent of φ and can thus be omitted. We can approximate the gradient of Equation 12 with

<!-- formula-not-decoded -->

where a t is evaluated at f φ ( ϵ t ; s t ) . This unbiased gradient estimator extends the DDPG style policy gradients (Lillicrap et al., 2015) to any tractable stochastic policy.
...
- Bhatnagar, S., Precup, D., Silver, D., Sutton, R. S., Maei, H. R., and Szepesv´ ari, C. Convergent temporal-difference learning with arbitrary smooth function approximation. In Advances in Neural Information Processing Systems (NIPS) , pp. 1204-1212, 2009.
...
Proof. Define the entropy augmented reward as r π ( s t , a t ) ≜ r ( s t , a t ) + E s t +1 ∼ p [ H ( π ( · | s t +1 ))] and rewrite the update rule as
...
Proof. Let π old ∈ Π and let Q π old and V π old be the corresponding soft state-action value and soft state value, and let π new be defined as

<!-- formula-not-decoded -->

It must be the case that J π ( π new ( s t )) J π ( π old ( s t )) , since we can always choose π new = π old Π
...
where we have repeatedly expanded Q π old on the RHS by applying the soft Bellman equation and the bound in Equation 18. Convergence to Q π new follows from Lemma 1.
...
Proof. Let π i be the policy at iteration i . By Lemma 2, the sequence Q π i is monotonically increasing. Since Q π is bounded above for π ∈ Π (both the reward and entropy are bounded), the sequence converges to some π ∗ . We will still need to show that π ∗ is indeed optimal. At convergence, it must be case that J π ∗ ( π ∗ ( · | s t )) &lt; J π ∗ ( π ( · | s t )) for all π ∈ Π , π = π ∗ . Using the same iterative argument as in the proof of Lemma 2, we get Q π ∗ ( s t , a t ) &gt; Q π ( s t , a t ) for all ( s t , a t ) ∈ S × A , that is, the soft value of any other policy in Π is lower than that of the converged policy. Hence π ∗ is optimal in Π .
...
Since the Jacobian d a / d u = diag(1 - tanh 2 ( u )) is diagonal, the log-likelihood has a simple form

<!-- formula-not-decoded -->

where u i is the i th element of u .
...
| Parameter                                                                                                                                                                             | Value                                                       |
|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|
| Shared optimizer learning rate discount ( γ ) replay buffer size number of hidden layers (all networks) number of hidden units per layer number of samples per minibatch nonlinearity | Adam (Kingma &Ba, 2015) 3 · 10 - 4 0.99 10 6 2 256 256 ReLU |
| SAC target smoothing coefficient ( τ ) target update interval gradient steps                                                                                                          | 0.005 1 1                                                   |
| SAC (hard target update) target smoothing coefficient ( τ ) target update interval gradient steps (except humanoids) gradient steps (humanoids)                                       | 1 1000 4 1                                                  |
